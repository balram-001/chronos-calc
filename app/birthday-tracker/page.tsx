...
export default function BirthdayTrackerPage() {
  ...
  const handleBdayCalculate = (e: any) => { 
    e.preventDefault(); 
    const today = new Date(2026, 6, 19); 
    let nextBday = new Date(2026, monthsMap[bdayMonth], Number(bdayDay)); 
    if (nextBday < today) nextBday.setFullYear(2027); 
    const diffDays = Math.ceil((nextBday.getTime() - today.getTime()) / (1000 * 60 * 60 * 24)); 
    setBdayResult({ months: Math.floor(diffDays / 30.4), days: Math.floor(diffDays % 30.4) }); 
  };
            ...
            <div className={`border rounded-2xl shadow-xl overflow-hidden ${darkMode ? "bg-slate-800 border-slate-700" : "bg-white border-gray-200"}`}>
              ...
              <div className="p-6 sm:p-8">
                <form onSubmit={handleBdayCalculate} className="flex flex-col gap-4">
                  ...
                  {bdayResult && <div className="mt-4 p-5 bg-red-50 dark:bg-slate-900/50 rounded-xl text-center border border-red-100"><span className="text-sm font-black text-red-600 uppercase tracking-tighter">{bdayResult.months}m {bdayResult.days}d to next milestone 🎂</span></div>}
                </form>
              </div>
            </div>
          </div>
