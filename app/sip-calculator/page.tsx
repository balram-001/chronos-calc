          ...
          <div className={`border rounded-2xl p-5 sm:p-8 shadow-xl ${darkMode ? "bg-slate-800 border-slate-700" : "bg-white border-gray-200"}`}>
            <div className="flex flex-col gap-5">
              <div>
                ...
                <input type="number" value={monthlyInvestment} onChange={(e) => setMonthlyInvestment(Number(e.target.value))} className={`w-full p-2.5 rounded-xl border text-sm font-bold ${darkMode ? "bg-slate-900 border-slate-700 text-white" : "bg-gray-50 border-gray-200"}`} />
              </div>
              <div>
                <label className="block text-[11px] font-black uppercase tracking-wider text-gray-400 mb-1">Expected Return Rate (% p.a.)</label>
                <input type="number" value={expectedReturn} onChange={(e) => setExpectedReturn(Number(e.target.value))} className={`w-full p-2.5 rounded-xl border text-sm font-bold ${darkMode ? "bg-slate-900 border-slate-700 text-white" : "bg-gray-50 border-gray-200"}`} />
              </div>
              <div>
                <label className="block text-[11px] font-black uppercase tracking-wider text-gray-400 mb-1">Time Period (Years)</label>
                <input type="number" value={timePeriod} onChange={(e) => setTimePeriod(Number(e.target.value))} className={`w-full p-2.5 rounded-xl border text-sm font-bold ${darkMode ? "bg-slate-900 border-slate-700 text-white" : "bg-gray-50 border-gray-200"}`} />
              </div>
            </div>
            ...
            <div className="grid grid-cols-3 gap-2 mt-8 pt-6 border-t border-gray-100 dark:border-slate-700 text-center">
              <div className={`p-3 rounded-xl border ${darkMode ? "bg-slate-900/50 border-slate-700" : "bg-gray-50 border-gray-100"}`}>
                <span className="text-[9px] uppercase font-black text-gray-400 block mb-1">Invested</span>
              ...
              <div className={`p-3 rounded-xl border ${darkMode ? "bg-slate-900/50 border-slate-700" : "bg-gray-50 border-gray-100"}`}>
                <span className="text-[9px] uppercase font-black text-gray-400 block mb-1">Est. Returns</span>
                <span className="text-xs sm:text-sm font-black text-[#5c940d]">₹{Math.round(wealthGained).toLocaleString("en-IN")}</span>
              </div>
              <div className={`p-3 rounded-xl border ${darkMode ? "bg-slate-900/50 border-slate-700" : "bg-gray-50 border-gray-100"}`}>
                <span className="text-[9px] uppercase font-black text-gray-400 block mb-1">Total Value</span>
